...
`/healthz` reports only process health. Use `/readyz` during deployment checks because it verifies database access, auth-provider schema access, login-template rendering, and session-cookie creation. A healthy deployed response includes `{"status":"ok","database":"ok","login_page":"ok","session":"ok"}`.
